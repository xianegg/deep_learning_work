...
import pandas as pd
import numpy as np
from tools.model import multiattribute_Model
from tools.utils import set_transform, timeSince
import tools.config as cfg
from tools.FS2K import get_loader


class Classifier_Trainer(object):

    def __init__(self, epochs, batch_size, learning_rate, model_type, pretrained=True):
        self.epochs = epochs  # 迭代次数
        self.batch_size = batch_size  # 批处理大小
        self.learning_rate = learning_rate  # 学习率
        self.selected_attrs = cfg.selected_attrs  # 分类的属性 头发|笑容｜性别..
        ...
        self.scheduler = optim.lr_scheduler.MultiStepLR(self.optimer, [30, 80], gamma=0.1)  # 调节学习率
    ...
    def train(self, epoch):
        ...
        for batch_idx, data in enumerate(self.train_loader):
            ...
            hair, hair_color, gender, earring, smile, frontal, style = self.model(images)  # 得到模型出来的结果
            # 计算各个属性的 交叉熵损失
            hair_loss = F.cross_entropy(input=hair, target=labels[0].to(self.device))
            hair_color_loss = F.cross_entropy(hair_color, target=labels[1].to(self.device))
            gender_loss = F.cross_entropy(gender, labels[2].to(self.device))
            earring_loss = F.cross_entropy(earring, labels[3].to(self.device))
            smile_loss = F.cross_entropy(smile, labels[4].to(self.device))
            frontal_loss = F.cross_entropy(frontal, labels[5].to(self.device))
            style_loss = F.cross_entropy(style, labels[6].to(self.device))
            # 总共的Loss
            total_loss = hair_loss + hair_color_loss + gender_loss + earring_loss + smile_loss + frontal_loss + style_loss
            ...
            if (batch_idx + 1) % (len(self.train_loader)//4) == 0:
                print("Epoch: %d/%d, training batch_idx:%d , time: %s, loss: %.4f" % (
                    epoch, self.epochs, batch_idx + 1, timeSince(self.start_time), total_loss.item()))
        # 返回epoch_loss
    ...
    def evaluate(self):  # 在测试集上进行评估性能
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(self.test_loader):
                ...
                hair, hair_color, gender, earring, smile, frontal, style = self.model(images)
    ...
    def fit(self, model_path=None):
        ...
        # 保存每个epoch的每个属性的正确率
        eval_acc_csv = pd.DataFrame(eval_acc_dict, index=[i for i in range(self.epochs)]).T
